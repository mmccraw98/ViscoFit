function [minimum,fval] = annealOpt(loss, parent, options, nelderopts,varargin)
% ANNEALOPT  Minimizes a function with the method of simulated annealing
% (Kirkpatrick et al., 1983) which has been modified to use the Nelder-Mead
% simplex method for optimization.
%
%  ANNEALOPT takes three input parameters, in this order:
%
...
%                  Default is 20.
%
%  NELDEROPTIONS is a structure with the settings passed to the nelder-mead
%  optimization function. It can be generated using the Matlab command
%  optimset(). Common modified settings include MaxFunEvals, MaxIter,
%  TolFun, and TolX. For documentation of these options, please see the
%  Matlab help database.
%
%  Usage:
%     [MINIMUM,FVAL] = ANNEALOPT(LOSS,NEWSOL,[OPTIONS],[NELDEROPTIONS],varargin);
...
%
%   Modified By:
%   cameron.parvini@gmail.com (3/27/2021)
...
if ~isempty(varargin)
    if length(varargin)>=2
        for i = 1:length(varargin)
            switch i
                case 1
                    ub = cell2mat(varargin{i});
                case 2
                    lb = cell2mat(varargin{i});
                otherwise
                    error('Too many arguments passed to annealOpt via varargin!')
            end
        end
    else
        fprintf('Only an upper bound was passed to annealOpt. Ignoring ub & lb.')
        ub = [];
        lb = [];
    end
else
...
while ~finished
    ...
    [newparam,newenergy,~] = fminsearch(loss, current, nelderopts);
    
    if ~isempty(ub)
        if ( any((ub - newparam) < 0) ) || ( any((newparam - lb) < 0) )
            newparam((ub - newparam) < 0) = ub((ub - newparam) < 0);
            newparam((newparam - lb) < 0) = lb((newparam - lb) < 0);
        end
    end
