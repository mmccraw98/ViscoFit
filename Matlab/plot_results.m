...
%             markerSize = (log10(max(simAbsMod))-3)*50;
            ...
            for k = 1:numel(tempResults.bestParams)
                ...
                switch lower(tempResults.model)
                    case 'maxwell'
                        ...
                        modelSSETime = sse_global(visco.forces,...
                ...
                hold on
                scatter(modelSSETime,modelSSEHarmonic,markerSize,markerType,'MarkerFaceColor',...
...
if Nplots > 1
    ...
    for i = 1:Nplots
        ...
        if any(i==(1:nCols:Nplots))
            ylabel('Absolute Modulus Error (SSE)','fontweight','bold')
        elseif any(i>(nRows-1)*nCols)
            xlabel('Time Series Error (SSE)','fontweight','bold')
        end
        set(gca,'xlim',[xlb xub])
